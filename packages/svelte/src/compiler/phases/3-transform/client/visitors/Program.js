...
import { build_getter, is_prop_source, apply_async_await_wrappers } from '../utils.js';
...
export function Program(node, context) {
	...
	if (context.state.analysis.instance && context.state.analysis.runes) {
		const [statements, target_statements] = apply_async_await_wrappers(node.body, context);
